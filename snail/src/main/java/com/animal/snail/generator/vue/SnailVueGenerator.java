package com.animal.snail.generator.vue;
...
import com.animal.snail.generator.SnailGenerator;
import com.animal.snail.generator.bean.Field;
...
@RequestMapping(value = "/snail/generate")
public class SnailVueGenerator {
    ...
    public String generate(String template, boolean api, String vueName, Integer rowNum, List<Field> fields) throws IOException {
        ...
        tableTemplate.binding("vueName", SnailGenerator.name(vueName, false));
        tableTemplate.binding("apiName", SnailGenerator.name(vueName, true));

        // 判断有无相关组件和日期范围搜索等组件
        Boolean upload = false, uploadThumb = false, quill = false, password = false, dict = false, customList = false,
                searchDict = false, searchCustomList = false;
        for (Field f : fields) {
        ...
        if ("tree.btl".equals(template)) {
            if (!containsField(fields, "snailTreeTitle")) {
                fields.add(0, new Field().setType("snailTreeTitle").setSortOrder(new BigDecimal("-1")).setEditable(true));
            }
            if (!containsField(fields, "snailTreeChoose")) {
                fields.add(0, new Field().setType("snailTreeChoose").setSortOrder(new BigDecimal("-2")).setEditable(true));
            }
            if (!containsField(fields, "snailTreeSortOrder")) {
                fields.add(new Field().setType("snailTreeSortOrder").setSortOrder(new BigDecimal("999")).setEditable(true));
            }
        }
        ...
        if (rowNum == 1) {
            ...
            throw new SnailException("rowNum仅支持数字1-4");
        }
        // 生成代码
