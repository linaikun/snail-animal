...
import com.animal.snail.modules.activiti.dao.ActProcessDao;
import com.animal.snail.modules.activiti.entity.ActBusiness;
import com.animal.snail.modules.activiti.entity.ActProcess;
import com.animal.snail.modules.activiti.service.ActNodeService;
import com.animal.snail.modules.activiti.service.ActProcessService;
import com.animal.snail.modules.activiti.service.ActStarterService;
import com.animal.snail.modules.activiti.utils.MessageUtil;
import com.animal.snail.modules.activiti.vo.ProcessNodeVo;
import com.animal.snail.modules.base.entity.DepartmentHeader;
import com.animal.snail.modules.base.entity.Role;
import com.animal.snail.modules.base.entity.User;
import com.animal.snail.modules.base.service.DepartmentHeaderService;
...
public class ActProcessServiceImpl implements ActProcessService {
        ...
        for (Task task : tasks) {
            if (actBusiness.getFirstGateway()) {
                ...
                if (users == null || users.isEmpty()) {
                    throw new SnailException("有任务节点未分配任何候选审批人，发起流程失败");
                } else {
                    // 分配了节点负责人分发给全部
                    for (User user : users) {
    ...
    public ProcessNodeVo getFirstNode(String procDefId) {
        ...
        FlowElement e = null;
        if (startEvent == null) {
            throw new SnailException("未找到该流程起始节点，请确认流程设计是否正确");
        }
        // 判断开始后的流向节点
        ...
        for (FlowElement element : elements) {
            if (element.getId().equals(sequenceFlow.getTargetRef())) {
                if (element instanceof UserTask) {
                    ...
                    throw new SnailException("流程设计错误，开始节点后只能是用户任务节点、排他网关、并行网关");
                }
            }
        }
        if (e == null) {
            throw new SnailException("未找到该流程起始后的任何节点，请确认流程设计是否正确");
        }
        // 排他、平行网关直接返回
        ...
        for (ActivityImpl activityImpl : activitiList) {
            if (activityImpl.getId().equals(currActId)) {
                ...
                if ("userTask".equals(type)) {
                    ...
                    throw new SnailException("流程设计错误，包含无法处理的节点");
                }
                break;
            }
        }
    ...
    public ProcessNodeVo getNode(String nodeId, String taskId) {
        ...
        if (actNodeService.hasCustomUser(nodeId)) {
            if(StrUtil.isBlank(taskId)){
                throw new SnailException("自选用户节点taskId不能为空");
            }
