...
public class LocalFileManage implements FileManage {
    ...
    public OssSetting getOssSetting() {
        ...
        if (setting == null || StrUtil.isBlank(setting.getValue())) {
            throw new SnailException("您还未配置本地存储");
        }
    ...
    public String inputStreamUpload(InputStream inputStream, String key, MultipartFile file) {
        ...
        if (f.exists()) {
            throw new SnailException("文件名已存在");
        }
        try {
            file.transferTo(f);
            return path + "/" + key;
            ...
            throw new SnailException("上传文件出错");
        }
    }
    ...
    public static void view(String url, HttpServletResponse response) {
        ...
        File file = new File(url);
        if (!file.exists()) {
            throw new SnailException("文件不存在");
        }
        ...
        try (FileInputStream is = new FileInputStream(file);
            ...
            log.error(e.toString());
            throw new SnailException("读取/下载文件出错");
        }
    }
}
