    ...
    <Card>
      <Row v-show="openSearch" @keydown.enter.native="handleSearch">          <Form ref="searchForm" :model="searchForm" inline :label-width="70">
        <FormItem label="名称" prop="cur_name">
          <Input type="text" v-model="searchForm.cur_name" placeholder="请输入名称" clearable style="width: 200px"/>
        </FormItem>
...
import { getCurriculumList, deleteCurriculum } from "@/api/index";
// 根据你的实际添加编辑组件位置路径修改
  ...
  methods: {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
          deleteCurriculum({ids: v.id}).then(res => {
            this.$Modal.remove();
            if (res.success) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          // 批量删除
          deleteCurriculum({ids: ids}).then(res => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.clearSelectAll();
