...
<template>
  <div class="search">
    ...
    <Card v-show="!showProcessNodeEdit">
      <Row v-show="openSearch" @keydown.enter.native="handleSearch">
        <Form ref="searchForm" :model="searchForm" inline :label-width="70">
          ...
          <FormItem label="标识Key" prop="processKey">
            <Input
              type="text"
      ...
      <Row class="operation">
        <Button @click="deploy" type="primary" icon="md-cloud-upload"
          >部署流程文件</Button
        >
      ...
      <Row v-show="openTip">
        <Alert show-icon>
          ...
          <span style="margin-left: 20px"
            >说明：当有多个相同标识的流程时，默认仅显示其最新版本</span
          >
        </Alert>
      </Row>
      <Row>
    ...
    <Modal :title="viewTitle" v-model="viewImage" draggable width="800">
      ...
      />
      <div slot="footer">
        <Button @click="viewImage = false" style="margin-right: 5px"
          >关闭</Button
        >
...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            if (params.row.status == 0) {
              // 挂起可激活
              suspendOrActive = h("DropdownItem", { props: { name: "active" } }, "激活");
            } else {
              // 激活可挂起
              suspendOrActive = h("DropdownItem", { props: { name: "suspend" } }, "挂起");
            }
            return h("div", [
              h(
                "a",
              ...
              h(
                ...
                [
                  ...
                  h("DropdownMenu", { slot: "list" }, [
                    suspendOrActive,
                    h("DropdownItem", { props: { name: "convert" } }, "转模型"),
  ...
  methods: {
    ...
    changeDropDown(row, v) {
      if (v == "active") {
        this.editStatus(1, row);
      } else if (v == "suspend") {
        this.editStatus(0, row);
      } else if (v == "convert") {
        this.convert(row);
      } else if (v == "remove") {
        this.remove(row);
